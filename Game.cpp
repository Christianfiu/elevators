...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    while (getline(gameFile, newLine)) {
        
        gameFile >> newLine;
       
        Person personCreated(newLine);
        
        // Determine tick the Person will spawn in
        int tickSpawned = personCreated.getTurn();
        
        // Have user play game until tick has finished
        ...
        building.spawnPerson(personCreated);
    }
}
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    // Check if each element is a digit in the range '0' through '9'
    for (int i = 0; i < pickupList.length(); i++) {
        if (pickupList.at(i) < '0' || pickupList.at(i) > '9') {
            return false;
        }
    }
   
    // Check if the length of pickupList is within the elevator capacity
    if (pickupList.length() > ELEVATOR_CAPACITY) {
        return false;
    }
    
    // Get the floor using pickupFloorNum
    Floor floor = building.getFloorByFloorNum(pickupFloorNum);

    // Check if the maximum value pointed to by an index is less
    // than the number of people on the floor
    for (int i = 0; i < pickupList.length(); i++) {
        int index = pickupList.at(i) - '0';  // Convert character to integer
        if (index >= floor.getNumPeople()) {
            return false;
        }
    }
    
    // Initialize targetDirection for the first person in pickupList
    char targetDirection;
    if (floor.getPersonByIndex(pickupList.at(0) - '0').getTargetFloor() > pickupFloorNum) {
        targetDirection = 'u';
    } else {
        targetDirection = 'd';
    }

    // Check if each person in pickupList is going in the same direction
    for (int i = 1; i < pickupList.length(); i++) {
        int index = pickupList.at(i) - '0';
        char direction;
        if (floor.getPersonByIndex(index).getTargetFloor() > pickupFloorNum) {
            direction = 'u';
        } else {
            direction = 'd';
        }
